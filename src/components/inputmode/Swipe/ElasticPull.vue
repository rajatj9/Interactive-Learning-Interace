        ...
        <g class="chart" :transform="chartTransform">
          ...
          <g :id="'histogram'+id" class="histogram">
            <rect class="bar-rect" v-for="(value, index) in minorBarsData"
                  :key="'bar-rect'+index" style="display: inline-block;" :x="xScale(value) - minorBarsWidth * 0.5"
          ...
          <g class="visual">
            ...
            <g class="baseline" v-show="showBaseline || isStandardValue">
              <line class="marking baseline" :id="'horizontal-baseline'+id"
                    :x1="xScale(0)" :x2="lineWidth"
                    :y1="yScale(Math.round(value))"
                    :y2="yScale(Math.round(value))"></line>
              <line class="marking baseline" :id="'vertical-baseline'+id"
                    :x1="xScale(Math.round(value))"
                    :x2="xScale(Math.round(value))"
                    :y1="yScale(standardMax)" :y2="yScale(0)"></line>
            </g>
            ...
            <g v-if="isAnswerPanel&&(value!=null)" :id="'ball'+id">
              <circle :id="'shapeBall'+id" class="swipeShapeBall"
                      :cx="answerPoint.x" :cy="answerPoint.y"
                      :r="controllerRadiusScale(value)"
                      filter="url(#lineShadow)"></circle>
              <circle :id="'scoreBall'+id" class="scoreBall"
                      :cx="answerPoint.x" :cy="answerPoint.y"
                      :r="radiusScale(value * 0.5)"></circle>
            </g>
          </g>
        </g>
      </svg>
...
<script>
  ...
  export default {
    name: "ElasticPull",
    mixins: [inputScaleMixin, sketchPadMixin, touchTipsMixin, chartMixin, minorBarMixin, elasticMixin],
  }
</script>
